...
async function testQueueJoinWithSession() {
    console.log('Testing Queue Join with Session ID...\n');
    console.log('Session ID:', testSessionId);
    console.log('Merchant ID:', testMerchantId);

    try {
        // Step 1: Join queue with sessionId
        console.log('\n1. Joining queue via /api/queues/join...');
        const joinResponse = await axios.post(`${BASE_URL}/api/queues/join`, {
            customerName: 'Test Customer',
            customerPhone: '+60123456789',
            partySize: 2,
            merchantId: testMerchantId,
            specialRequests: 'Testing session flow',
        ...
        console.log('The session is properly linked and status checks work correctly.');
